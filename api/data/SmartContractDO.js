...
class SmartContractDO {
  ...
  async getBalance(address) {
    try {
      ...
      const contract = await new this.web3.eth.Contract(flappyabi, this.token_address);
  ...
  async withdraw(address, amount){
    ...
    const rs = await vault_contract.methods.withdraw(value, address)
      .send({ from: this.withdrawer_address, gas: 3000000 });
